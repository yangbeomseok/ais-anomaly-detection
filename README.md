...
- Isolation Forest(글로벌) / LOF(선박별) / HDBSCAN(선박별) 세 가지 모델로 이상 탐지 돌려서
...
LOF와 HDBSCAN은 선박별(per-vessel)로 실행합니다. 273만 행을 한번에 돌리면 LOF에서 동일 거리 문제로 수치 오류가 발생하고, HDBSCAN은 메모리 부족이 일어나기 때문입니다. 선박별 실행은 각 선박의 자체 행동 패턴 대비 이상을 탐지하므로 의미론적으로도 더 적합합니다.
...
3가지 모델을 돌렸습니다. Isolation Forest(글로벌)와 LOF(선박별)는 contamination=5%로 설정했고, HDBSCAN(선박별)은 밀도 기반으로 자체 판단합니다.

| 모델 | 이상 탐지 수 | 비율 | 방식 |
|------|-------------|------|------|
| Isolation Forest | 136,528 | 5.00% | 글로벌 |
| LOF | 136,655 | 5.00% | 선박별 |
| HDBSCAN | 445,466 | 16.31% | 선박별 |
| **앙상블 (2/3 동의)** | **145,804** | **5.34%** | — |

HDBSCAN이 16%로 높은 이유는 선박별로 min_cluster_size를 적응적으로 설정하면서 노이즈(클러스터에 속하지 않는 점)가 많이 잡혔기 때문입니다. 앙상블에서 단일 모델만 이상이라고 한 건은 걸러내므로 false positive가 줄어듭니다.

### 4-1. 모델 간 겹침 분석

![Model Overlap](results/figures/model_overlap.png)

3개 모델이 어떻게 겹치는지 보면, HDBSCAN 단독이 305,896건으로 가장 많고, 3개 모두 동의한 건은 14,613건입니다. IF와 LOF만 겹치는 건은 6,234건으로 적은데, 이 두 모델이 다른 관점(글로벌 vs 선박별)으로 접근하기 때문입니다.

### 4-2. contamination 민감도 분석
...
이상 레코드가 가장 많은 선박(MMSI: 367004580, 총 8,962건 중 1,822건 이상) 한 척을 7일간 추적한 시계열입니다. 위에서부터 SOG(속도), COG(침로), Course Change(침로 변화량)입니다. 빨간 점이 이상으로 탐지된 시점인데, 급격한 속도/침로 변화 구간에서 이상이 집중됩니다.
...
UTC 기준 시간대별 이상 발생률입니다. 야간(22~23시 UTC)에 이상 비율이 ~8.7%로 가장 높고, 오전(10시)에 ~3.5%로 가장 낮습니다. 야간에 이상 행동이 집중되는 패턴은 불법 활동과 관련될 수 있습니다.

### 11. 선박 유형별 이상 비율

![Anomaly Rate by Type](results/figures/anomaly_rate_by_type.png)

선종별 이상 비율입니다. Passenger(8.4%)가 가장 높고 Tanker(2.2%)가 가장 낮습니다. 여객선이 높은 건 운항 특성상 항구 입출항 시 급가속/급감속이 잦기 때문으로 보입니다. 탱커는 대형 선박 특성상 안정적인 항행 패턴을 보여 이상 비율이 낮습니다.
...
7일간 AIS 레코드(273만 행)에서 3개 모델 앙상블로 145,804건(5.34%)의 이상 행동을 탐지했습니다. SHAP으로 뜯어보니 이상 판정에 가장 크게 기여하는 피처는 **침로 변화(course_change)**와 **정박 패턴(stop_duration_min)**, 그리고 **속도(SOG)**였습니다.
