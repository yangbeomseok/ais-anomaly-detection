...
실제로 불법조업 선박은 단속 해역에서 급격한 방향 전환을 하거나, AIS 신호를 꺼버리는 패턴을 보입니다. 이런 행동 패턴을 비지도 학습으로 탐지하는 게 프로젝트의 핵심입ㄴ다.
...
| DBSCAN | 밀도 기반 클러스터링. 어떤 클러스터에도 속하지 않는 점을 이상치(noise)로 분류 |
| Permutation Importance | 피처 하나를 랜덤으로 섞었을 때 모델 성능이 얼마나 떨어지는지로 중요도를 측정하는 방법 |
...
contamination(오염 비율)을 1%~10%로 바꿔보면서 결과가 얼마나 달라지는지 확인했습니다.

| contamination | IF | LOF | 앙상블 (2/3) | 비율 |
|:---:|---:|---:|---:|:---:|
| 0.01 | 6,296 | 6,284 | 1,445 | 0.23% |
| 0.03 | 18,886 | 18,886 | 2,888 | 0.46% |
| **0.05** | **31,477** | **31,477** | **4,907** | **0.78%** |
| 0.07 | 44,066 | 44,067 | 7,434 | 1.18% |
| 0.10 | 62,953 | 62,953 | 12,678 | 2.01% |

IF와 LOF의 결과가 거의 같은 건 contamination 값이 변해도 동일합니다. 앙상블이 중요한 이유가 여기서 나오는데, 단일 모델은 contamination에 선형적으로 반응하지만, 2/3 동의 기준을 적용하면 증가 폭이 완화됩니다. 0.05를 선택한 건 이상 탐지 문헌에서 일반적으로 사용하는 범위(1~10%)의 중간값이기 때문입니다.
...
63만 건의 AIS 레코드에서 3개 모델 앙상블로 **4,907건(0.78%)**의 이상 행동을 탐지했습니다. 이상의 대부분은 두 가지 유형으로 나뉩니다.

| 이상 유형 | 건수 | 주요 발생 위치 |
|-----------|------|---------------|
| 속도 이상 (급가속/급감속) | 1,554건 | 연안 전역 |
| 급회전 (침로 변화 > 90도) | 1,422건 | 항구 입출항 구간 |
| 장시간 신호 단절 | 0건 | — (활동량 상위 500척 한정) |

피처 중요도 분석에서도 **SOG(속도)**와 **course_change(침로 변화)**가 이상 판별에 가장 큰 영향을 주는 것으로 확인되었습니다. "갑자기 빨라지거나 급회전하는 배"가 통계적으로도 가장 이상한 배입니다.

### 실용적 의미

이 시스템의 핵심 가치는 **스크리닝**입니다. 하루치 720만 건 → 상위 500척 63만 건 → **최종 4,907건**으로 감시 대상을 좁힐 수 있습니다. 해양 감시 인력이 전수조사 대신 0.78%의 우선순위 목록에 집중할 수 있다는 뜻입니다.

딥다이브 분석(MMSI: 416497000)에서 확인한 것처럼, 이상 레코드는 출항 직후 급가속 구간이나 항로 전환 지점에 집중됩니다. 이 중 일부는 정상적인 운항 패턴(출항 가속, 항구 접안 회전)이고, 일부는 실제 수상한 행동일 수 있습니다. 비지도 학습은 이 둘을 구분하지 못하지만, **"여기를 먼저 봐라"**는 우선순위를 제공합니다.

### 한국 해역 적용 시

현재 분석은 미국 연안 데이터로 방법론을 검증한 단계입니다. 한국 해역에 적용하면 다른 결과가 나올 수 있는 지점들이 있습니다.

- **불법조업 탐지**: 한국 EEZ 내 중국 어선의 불법조업이 실제 이슈인 만큼, 어선의 이상 비율이 미국 데이터(0.6%)보다 높게 나올 가능성이 있습니다
- **신호 단절 패턴**: 상위 500척에서는 0건이었지만, 불법 선박은 단속 회피를 위해 AIS를 의도적으로 끄는 경우가 많아 전체 선박 분석 시 핵심 피처가 될 수 있습니다
- **야간 집중 패턴**: 미국 데이터에서는 시차 분산으로 야간 패턴이 희석되었지만, 단일 시간대인 한국 해역에서는 야간 이상 집중이 더 뚜렷하게 나타날 것으로 예상됩니다
