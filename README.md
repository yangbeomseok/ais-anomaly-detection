...
선박들은 해양에서 AIS 신호를 통해 위치, 속도, 침로 등을 주기적으로 송신하는데, 이걸 역으로 활용하면 **"이 배 뭔가 이상한데?"** 를 데이터로 잡아낼 수 있지 않을까? 라는 생각이 출발점이었습니다.
...
분석에서 자주 나오는 용어들을 정리해두었습니다. (AI 적극 활용)
...
같은 선박의 항적을 지도에 찍으면 이렇게 됩니다. 이 선박은 루이지애나 내륙 수로(미시시피강 하구)에서 활동하며, 이동 범위가 약 1~4km로 매우 좁습니다. 이상 레코드가 많이 잡힌 이유는 **항구/수로에서의 반복적인 정박-미세이동 패턴**이 모델에 의해 이상으로 탐지되었기 때문입니다. Top 3 이상 선박이 모두 내륙 수로(루이지애나, 오하이오강) 선박인 점은, 해양 이상 탐지 시 내륙 수로 선박을 별도 처리하거나 필터링해야 할 수 있음을 시사합니다.
